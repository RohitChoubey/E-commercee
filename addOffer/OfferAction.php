...
// initializing variables
  ...
  if (empty($save)) { array_push($errors, "save is required"); }



  // first check the database to make sure 
  // a user does not already exist with the same username and/or email
  $user_check_query = "SELECT * FROM offer WHERE productname='$productname' OR title='$title' LIMIT 1";
...
// Allow certain file formats 
  ...
  // Finally, register user if there are no errors in the form
	if (count($errors) == 0 && in_array($fileType, $allowTypes)) {
		...
		$query = "INSERT INTO offer (fimg, bimg, productname, title, price, stock, dis, sip, war, save ,mrp) 
		VALUES('$imgContent', '$imgContentt', '$productname', '$title', '$price', '$stock', '$dis', '$sip', '$war','$save','$mrprice')";
		mysqli_query($db, $query);
		$_SESSION['productname'] = $productname;
		$_SESSION['success'] = "Your o offer Added Sucessfully";
