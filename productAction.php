...
// initializing variables
  ...
  if (empty($war)) { array_push($errors, "Type of Product is required"); }



  // first check the database to make sure 
  // a user does not already exist with the same username and/or email
  $user_check_query = "SELECT * FROM product WHERE productname='$productname' OR title='$title' LIMIT 1";
...
// Allow certain file formats 
  ...
  // Finally, register user if there are no errors in the form
	if (count($errors) == 0 && in_array($fileType, $allowTypes)) {
		...
		$query = "INSERT INTO product (fimg, bimg, productname, title, price, stock, dis, sip, war,mrp,type,save) 
		VALUES('$imgContent', '$imgContentt', '$productname', '$title', '$price', '$stock', '$dis', '$sip', '$war','$mrprice','$type',NOW(),'$save')";
		mysqli_query($db, $query);
		$_SESSION['productname'] = $productname;
		$_SESSION['success'] = "Your Product is Added Sucessfully";
		header("Location: add/addProduct.php"); 
